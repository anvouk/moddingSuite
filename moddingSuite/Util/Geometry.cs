...
public static class Geometry
{
    ...
    {
        List<AreaVertex> outline =
            content.Vertices.GetRange(content.BorderVertex.StartVertex, content.BorderVertex.VertexCount);
    ...
    public static AreaContent getFromOutline(List<AreaVertex> outline)
    {
        ...
        if (!isRightHanded(outline)) outline.Reverse();
        outline = outline.Select(x =>
        {
            x.Center = 1;
            return x;
        }).ToList();
    ...
    public static bool isInside(AreaVertex v1, List<AreaVertex> outline)
    {
        ...
        for (int i = 0; i < outline.Count; i++)
        {
            ...
            int k = i + 1;
            if (i == outline.Count - 1) k = 0;
            AreaVertex b2 = outline.ElementAt(k);
            if (v1.Equals(b1) || v1.Equals(b2) || v2.Equals(b1) || v2.Equals(b2)) continue;
            if (isRightHanded(new List<AreaVertex> { v1, b1, b2 }) ==
                isRightHanded(new List<AreaVertex> { v2, b1, b2 }))
                continue;
            if (isRightHanded(new List<AreaVertex> { v1, v2, b1 }) ==
                isRightHanded(new List<AreaVertex> { v1, v2, b2 }))
                continue;
            crossings++;
        }
    ...
    private static List<AreaVertex> getInnerRing(List<AreaVertex> outline)
    {
        ...
        {
            int im = i - 1;
            if (im < 0) im = outline.Count - 1;
            int ip = i + 1;
            if (ip == outline.Count) ip = 0;
            double localAngle =
                (getAngle(outline.ElementAt(im), outline.ElementAt(i), outline.ElementAt(ip)) + Math.PI) / 2;
    ...
    private static bool isRightHanded(List<AreaVertex> polygon)
    {
        ...
        {
            int im = i - 1;
            if (im < 0) im = polygon.Count - 1;
            int ip = i + 1;
            if (ip == polygon.Count) ip = 0;
