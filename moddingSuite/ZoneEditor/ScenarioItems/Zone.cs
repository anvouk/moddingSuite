...
public class Zone : ScenarioItem
{
    ...
    public Zone(Editor e, Point p, int index)
    {
        ...
        attachPoint.Colour = Brushes.Green;
    ...
    protected override void paint(object sen, PaintEventArgs e)
    {
        ...
        Font font = new(FontFamily.GenericSansSerif, 16);
        Point namePos = attachPoint.getPosition();
        namePos.Offset(-(10 * Name.Length) / 2, -font.Height);
        e.Graphics.DrawString(Name, font, Brushes.White, namePos);
        Point valPos = attachPoint.getPosition();
        string valString = string.Format("{0}", value);
        valPos.Offset(-(10 * valString.Length) / 2, 0);
        e.Graphics.DrawString(valString, font, Brushes.White, valPos);
    }

    public List<AreaVertex> getRawOutline()
    {
        if (area != null)
            return area.Content.Vertices.GetRange(area.Content.BorderVertex.StartVertex,
                area.Content.BorderVertex.VertexCount);
    ...
    public override void buildNdf(NdfBinary data, ref int i)
    {
        ...
        Point3D p = Geometry.Geometry.convertPoint(attachPoint.getPosition());
