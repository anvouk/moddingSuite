...
public class Spawn : ScenarioItem
{
    ...
    public Spawn(Point p, float rot, float scale, int i, SpawnType t)
    {
        ...
        p.Offset(-(int)(scale * arrowLength * Math.Cos(rot) / Geometry.Geometry.scaleFactor),
            -(int)(scale * arrowLength * Math.Sin(rot) / Geometry.Geometry.scaleFactor));
    ...
    protected override void paint(object sen, PaintEventArgs e)
    {
        ...
        ahBase.Offset(-(int)(width * arrowHeadLength * Math.Cos(rot) / Geometry.Geometry.scaleFactor),
            -(int)(width * arrowHeadLength * Math.Sin(rot) / Geometry.Geometry.scaleFactor));
        Point ahLeft = ahBase;
        rot += (float)Math.PI / 2;
        ahLeft.Offset(-(int)(width * arrowHeadLength * Math.Cos(rot) / Geometry.Geometry.scaleFactor),
            -(int)(width * arrowHeadLength * Math.Sin(rot) / Geometry.Geometry.scaleFactor));
        Point ahRight = ahBase;
        rot -= (float)Math.PI;
        ahRight.Offset(-(int)(width * arrowHeadLength * Math.Cos(rot) / Geometry.Geometry.scaleFactor),
            -(int)(width * arrowHeadLength * Math.Sin(rot) / Geometry.Geometry.scaleFactor));
        rot += (float)Math.PI / 2;
        ahBase.Offset((int)(500 * width * Math.Cos(rot) / Geometry.Geometry.scaleFactor),
            (int)(500 * width * Math.Sin(rot) / Geometry.Geometry.scaleFactor));
        e.Graphics.DrawLine(p, source.getPosition(), ahBase);
        e.Graphics.FillPolygon(b, new[] { ah, ahLeft, ahRight });
    }
    ...
    public override void buildNdf(NdfBinary data, ref int i)
    {
        ...
        scaleProperty.Value = new NdfVector(new Point3D(xScale, 1, 1));
