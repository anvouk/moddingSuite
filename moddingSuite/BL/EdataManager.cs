...
public class EdataManager : ViewModelBase
{
    ...
    static EdataManager()
    {
        ...
        {
            new(EdataFileType.Ndfbin, ndfbinheader),
            new(EdataFileType.Package, edataHeader),
            new(EdataFileType.Dictionary, tradHeader),
            new(EdataFileType.Save, savHeader),
            new(EdataFileType.Image, tgvHeader),
            new(EdataFileType.Mesh, meshHeader),
            new(EdataFileType.Scenario, scenarioHeader)
        };
    }
    ...
    public void ParseEdataFile()
    {
        ...
        switch (Header.Version)
        {
            ...
            case 2:
                ...
                break;
            default:
                throw new NotSupportedException(string.Format("Edata Version {0} is currently not supported",
                    Header.Version));
        }
    }
    ...
    /// <summary>
    ///     The only tricky part about that algorythm is that you have to skip one byte if the length of the File/Dir name PLUS
    ///     nullbyte is an odd number.
    /// </summary>
    ...
    public static EdataFileType GetFileTypeFromHeaderData(byte[] headerData)
    {
        foreach (KeyValuePair<EdataFileType, byte[]> knownHeader in KnownHeaders)
            ...
            else if (Utils.ByteArrayCompare(headerData, knownHeader.Value))
            {
                return knownHeader.Key;
            }
