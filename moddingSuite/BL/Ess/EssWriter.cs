...
public class EssWriter
{
    ...
    public byte[] WriteEss(byte[] data)
    {
        ...
        using (MemoryStream fs = new MemoryStream(data))
        {
            ...
            if (id == 16) // old wav
            {
                ...
                br.ReadUInt16();
                id = br.ReadUInt16();
                if (id != 16) throw new InvalidDataException($"Only 16 bit WAV supported. {id} was given.");
            }
    ...
    public byte[] Write(byte[] data)
    {
        ...
        else if (id == 40) // new wav
        {
            ...
            br.ReadUInt16();
            id = br.ReadUInt16();
            if (id != 16) throw new InvalidDataException("Only 16 bit WAV supported.");
            br.ReadUInt32();
            br.ReadUInt32();
            br.ReadUInt32();
            br.ReadUInt32();
        ...
        id = br.ReadUInt32();
        if (id != 0x61746164) throw new InvalidDataException("Data not found.");

        if (channels != 1) throw new InvalidDataException("Only mono files supported.");
        int nsamples = br.ReadInt32() / channels / 2;
