...
public static class TextBoxAutoSelect
{
    ...
    public static readonly DependencyProperty IsEnabledProperty = DependencyProperty.RegisterAttached("IsEnabled",
        typeof(bool), typeof(TextBoxAutoSelect), new PropertyMetadata(false, PropertyChangedCallback));

    private static void PropertyChangedCallback(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        if (d is Control control) control.Loaded += Handler;
    }
    ...
    private static void ControlOnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        {
            control.Focus();
            if (control is TextBoxBase textbox) textbox.SelectAll();
